...
function addSong(s) {
  ...
  renderQueue(); msg('searchMessage','«'+s.track+'» añadida. Añade su enlace exacto desde «Tu selección». ');
}
function renderQueue() {
  ...
  state.songs.forEach((s,i)=>{
    const row=el('div','queue-item');if(!s.uri)row.classList.add('missing-link');
  ...
  const missing=state.songs.filter(song=>!song.uri).length;
  msg('linkProgress',!state.songs.length?'Empieza añadiendo una canción. Después pega su enlace de Spotify.':missing?`${missing} ${missing===1?'canción necesita':'canciones necesitan'} su enlace exacto de Spotify. Usa «Añadir enlace» en la lista.`:'¡Todas las canciones tienen enlace! Revisa las fechas y descarga.');
